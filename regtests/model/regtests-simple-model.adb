...
--  Ada Generator: https://ada-gen.googlecode.com/svn/trunk Revision 70
-----------------------------------------------------------------------
...
package body Regtests.Simple.Model is
   ...
   procedure Save (Object  : in out Allocate_Impl;
                   Session : in out ADO.Sessions.Master_Session'Class) is
      Stmt : ADO.Statements.Update_Statement := Session.Create_Statement (ALLOCATE_TABLE'Access);
   begin
      if Object.Is_Modified (1) then
   ...
   procedure Delete (Object  : in out Allocate_Impl;
                     Session : in out ADO.Sessions.Master_Session'Class) is
      Stmt : ADO.Statements.Delete_Statement := Session.Create_Statement (ALLOCATE_TABLE'Access);
   begin
      Stmt.Set_Filter (Filter => "id = ?");
      Stmt.Add_Param (Value => Object.Get_Key);
      Stmt.Execute;
   ...
   function Get_Value (Item : in Allocate_Ref;
                       Name : in String) return Util.Beans.Objects.Object is
      Impl : constant access Allocate_Impl := Allocate_Impl (Item.Get_Load_Object.all)'Access;
   begin
      if Name = "id" then
         return ADO.Objects.To_Object (Impl.Get_Key);
      end if;
